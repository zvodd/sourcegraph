...
(function() {
    ...
    function F9(fun)
    {
      function wrapper(a) { return function(b) { return function(c) {
        return function(d) { return function(e) { return function(f) {
        return function(g) { return function(h) { return function(index) {
        return fun(a, b, c, d, e, f, g, h, index); }; }; }; }; }; }; }; };
      }
      wrapper.arity = 9;
      wrapper.func = fun;
      return wrapper;
    ...
    function A9(fun, a, b, c, d, e, f, g, h, index)
    {
      return fun.arity === 9
        ? fun.func(a, b, c, d, e, f, g, h, index)
        : fun(a)(b)(c)(d)(e)(f)(g)(h)(index);
    }
    ...
    function get(index, array)
    {
        ...
        {
            throw new Error(
                'Index ' + index + ' is out of range. Check the length of ' +
                'your array first or use getMaybe or getWithDefault.');
        }
        return unsafeGet(index, array);
    }
    
    function unsafeGet(index, array)
    {
        for (var x = array.height; x > 0; x--)
        {
            var slot = index >> (x * 5);
            while (array.lengths[slot] <= index)
            {
                slot++;
            }
            if (slot > 0)
            {
                index -= array.lengths[slot - 1];
            }
    ...
    // copied and updated.
    function set(index, item, array)
    {
        if (index < 0 || length(array) <= index)
        {
            return array;
        }
        return unsafeSet(index, item, array);
    }
    
    function unsafeSet(index, item, array)
    {
        array = nodeCopy(array);
    
        if (array.height === 0)
        {
            array.table[index] = item;
        }
        else
        {
            var slot = getSlot(index, array);
            if (slot > 0)
            {
                index -= array.lengths[slot - 1];
            }
            array.table[slot] = unsafeSet(index, item, array.table[slot]);
        }
        return array;
    }
    
    function initialize(length_, f)
    {
        if (length_ <= 0)
        {
            return empty;
        }
        var h = Math.floor( Math.log(length_) / Math.log(M) );
        return initialize_(f, h, 0, length_);
    }
    ...
    function initialize_(f, h, from, to)
    {
        ...
        for (var index = 0; index < table.length; index++)
        {
            table[index] = initialize_(f, h - 1, from + (index * step), Math.min(from + ((index + 1) * step), to));
            lengths[index] = length(table[index]) + (index > 0 ? lengths[index-1] : 0);
        }
        return {
            ctor: '_Array',
            height: h,
    ...
    function fromListPush(toPush, nodes)
    {
        ...
        let length_ = length(toPush);
        if (nodes[h].lengths.length > 0)
        {
            length_ += nodes[h].lengths[nodes[h].lengths.length - 1];
        }
        nodes[h].lengths.push(length_);
    ...
    {
        let pushed = push_(item, a);
        if (pushed !== null)
        {
            return pushed;
        }
    
        let newTree = create(item, a.height);
    ...
    {
        for (let index = a.table.length - 1; index >= 0; index--)
        {
            list =
                a.height === 0
                    ? _elm_lang$core$Native_List.Cons(a.table[index], list)
                    : toList_(list, a.table[index]);
        }
        return list;
    }
    ...
    function indexedMap_(f, a, from)
    {
        ...
        for (let index = 0; index < a.table.length; index++)
        {
            newA.table[index] =
                a.height === 0
                    ? A2(f, from + index, a.table[index])
                    : indexedMap_(f, a.table[index], index == 0 ? from : from + a.lengths[index - 1]);
        }
        return newA;
    }
    ...
    function foldl(f, b, a)
    {
        if (a.height === 0)
        {
            for (var index = 0; index < a.table.length; index++)
            {
                b = A2(f, a.table[index], b);
            }
        }
        else
        {
    ...
    function foldr(f, b, a)
    {
        if (a.height === 0)
        {
            for (var index = a.table.length; index--; )
            {
                b = A2(f, a.table[index], b);
            }
        }
    ...
    function sliceRight(to, a)
    {
        ...
        let right = getSlot(to, a);
        let sliced = sliceRight(to - (right > 0 ? a.lengths[right - 1] : 0), a.table[right]);
    ...
    function sliceLeft(from, a)
    {
        ...
        let left = getSlot(from, a);
        let sliced = sliceLeft(from - (left > 0 ? a.lengths[left - 1] : 0), a.table[left]);
        ...
        {
            length_ += length(newA.table[i]);
            newA.lengths[i] = length_;
        }
    ...
    function append(a,b)
    {
        ...
        if (c[0].table.length + c[1].table.length <= M)
        {
            ...
            if (c[0].height > 0)
            {
                let length_ = length(c[0]);
                for (var i = 0; i < c[1].lengths.length; i++)
                {
                    c[1].lengths[i] += length_;
                }
    ...
    function append_(a, b)
    {
        ...
        if (a.height !== 1 || b.height !== 1)
        {
            ...
            else
            {
                ...
                let left = appended[0].table.length === 0 ? 0 : 1;
                let right = left === 0 ? 1 : 0;
    ...
    {
        let index = parent.table.length - 1;
    ...
    function insertLeft(parent, node)
    {
        if (node.table.length > 0)
        {
            ...
            let length_ = length(parent.table[0]);
            for (var i = 1; i < parent.lengths.length; i++)
            {
                length_ += length(parent.table[i]);
                parent.lengths[i] = length_;
            }
        }
        else
        {
    ...
    {
        let subscriptionLengths = 0;
        for (var i = 0; i < a.table.length; i++)
        {
            subscriptionLengths += a.table[i].table.length;
        }
        for (var i = 0; i < b.table.length; i++)
        {
            subscriptionLengths += b.table[i].table.length;
        }
    
        var toRemove = a.table.length + b.table.length;
        return toRemove - (Math.floor((subscriptionLengths - 1) / M) + 1);
    }
    ...
    function saveSlot(a, b, index, slot)
    {
        ...
        let l = (index === 0 || index === a.lengths.length)
            ? 0
    ...
    {
        let newA = createNode(a.height, Math.min(M, a.table.length + b.table.length - toRemove));
        let newB = createNode(a.height, newA.table.length - (a.table.length + b.table.length - toRemove));
        ...
        let write = read;
        let slot = new createNode(a.height - 1, 0);
        let from = 0;
        ...
        while (read - write - (slot.table.length > 0 ? 1 : 0) < toRemove)
        {
            ...
            let source = get2(a.table, b.table, read);
            let to = Math.min(M - slot.table.length, source.table.length);
    
            // Copy and adjust size table.
            slot.table = slot.table.concat(source.table.slice(from, to));
            if (slot.height > 0)
            {
                let length_ = slot.lengths.length;
                for (var i = length_; i < length_ + to - from; i++)
                {
    ...
    function getSlot(index, a)
    {
        var slot = index >> (5 * a.height);
        while (a.lengths[slot] <= index)
        {
            slot++;
        }
        return slot;
    ...
    {
        let jsArray = new Array(length(a));
        toJSArray_(jsArray, 0, a);
        return jsArray;
    }
    
    function toJSArray_(jsArray, index, a)
    {
        for (var t = 0; t < a.table.length; t++)
        {
            if (a.height === 0)
            {
                jsArray[index + t] = a.table[t];
            }
            else
            {
                var inc = t === 0 ? 0 : a.lengths[t - 1];
                toJSArray_(jsArray, index + inc, a.table[t]);
            }
        }
    }
    ...
    function fromJSArray(jsArray)
    {
        ...
        let h = Math.floor(Math.log(jsArray.length) / Math.log(M));
    ...
    function fromJSArray_(jsArray, h, from, to)
    {
        ...
        let step = Math.pow(M, h);
        let table = new Array(Math.ceil((to - from) / step));
        let lengths = new Array(table.length);
        for (let index = 0; index < table.length; index++)
        {
            table[index] = fromJSArray_(jsArray, h - 1, from + (index * step), Math.min(from + ((index + 1) * step), to));
            lengths[index] = length(table[index]) + (index > 0 ? lengths[index - 1] : 0);
        }
        return {
            ctor: '_Array',
    ...
    function module_(a, b)
    {
        ...
        var r = a % b;
        var m = a === 0 ? 0 : (b > 0 ? (a >= 0 ? r : r + b) : -module_(-a, -b));
    ...
    function eq(rootX, rootY)
    {
        ...
        while (stack.length > 0)
        {
            ...
            {
                throw new TypeError('Equality error: general function equality is ' +
    ...
    function map4(f, ws, xs, ys, zs)
    {
        ...
        while (   ws.ctor !== '[]'
               ...
               && zs.ctor !== '[]')
        {
            array.push(A4(f, ws._0, xs._0, ys._0, zs._0));
            ws = ws._1;
            xs = xs._1;
            ys = ys._1;
            zs = zs._1;
    ...
    function map5(f, vs, ws, xs, ys, zs)
    {
        ...
        while (   vs.ctor !== '[]'
               ...
               && zs.ctor !== '[]')
        {
            array.push(A5(f, vs._0, ws._0, xs._0, ys._0, zs._0));
            vs = vs._1;
            ws = ws._1;
            xs = xs._1;
            ys = ys._1;
        ...
        (result, n, value) => {
            ...
            while (true) {
                if (_elm_lang$core$Native_Utils.cmp(n, 0) < 1) {
                    ...
                    let _v23 = A2(_elm_lang$core$List_ops['::'], value, result);
                        var _v24 = n - 1;
                        var _v25 = value;
                    result = _v23;
                    n = _v24;
    ...
    return {
        ...
        toCode(c) { return c.charCodeAt(0); },
        toUpper(c) { return _elm_lang$core$Native_Utils.chr(c.toUpperCase()); },
        toLower(c) { return _elm_lang$core$Native_Utils.chr(c.toLowerCase()); },
        toLocaleUpper(c) { return _elm_lang$core$Native_Utils.chr(c.toLocaleUpperCase()); },
        toLocaleLower(c) { return _elm_lang$core$Native_Utils.chr(c.toLocaleLowerCase()); }
    };
    ...
    function spawn(task)
    {
        return nativeBinding((callback) => {
            let process = rawSpawn(task);
            callback(succeed(process));
        });
    }
    
    function rawSend(process, message)
    {
    ...
    function step(numberSteps, process)
    {
        while (numberSteps < MAX_STEPS)
        {
        ...
        if (numberSteps < MAX_STEPS)
        {
            return numberSteps + 1;
        }
        enqueue(process);
    ...
    function work()
    {
        let numberSteps = 0;
        var process;
        while (numberSteps < MAX_STEPS && (process = workQueue.shift()))
        {
            numberSteps = step(numberSteps, process);
        }
        if (!process)
        {
            working = false;
    ...
    {
        let init = main ? makeEmbed(name, main) : mainIsUndefined(name);
    ...
    function mainIsUndefined(name)
    {
        ...
        {
            let message = 'Cannot initialize module `' + name +
    ...
    function makeEmbed(moduleName, main)
    {
        return function embed(rootDomNode, flags, withRenderer)
        {
            try
            {
                let program = mainToProgram(moduleName, main);
                if (!withRenderer)
                {
            ...
            {
                rootDomNode.innerHTML = errorHtml(error.message);
                throw error;
            }
        };
    }
    ...
    {
        let main = wrappedMain.main;
    
        if (typeof main.init === 'undefined')
        {
            let emptyBag = batch(_elm_lang$core$Native_List.Nil);
            let noChange = _elm_lang$core$Native_Utils.Tuple2(
                _elm_lang$core$Native_Utils.Tuple0,
                emptyBag
            );
    
            return _elm_lang$virtual_dom$VirtualDom$programWithFlags({
                init() { return noChange; },
                view() { return main; },
                update: F2(() => { return noChange; }),
                subscriptions () { return emptyBag; }
            });
        }
    
        let flags = wrappedMain.flags;
        let init = flags
    ...
    function initWithFlags(moduleName, realInit, flagDecoder)
    {
        return function init(flags)
        {
            let result = A2(_elm_lang$core$Native_Json.run, flagDecoder, flags);
            if (result.ctor === 'Err')
            {
                throw new Error(
    ...
    function makeEmbedHelp(moduleName, program, rootDomNode, flags)
    {
        ...
        let makeRenderer = program.renderer;
        ...
        let initApp = _elm_lang$core$Native_Scheduler.nativeBinding((callback) => {
            let results = init(flags);
            let model = results._0;
            renderer = makeRenderer(rootDomNode, enqueue, view(model));
            let commands = results._1;
            var subs = subscriptions(model);
            dispatchEffects(managers, commands, subs);
        ...
        function onMessage(message, model)
        {
            return _elm_lang$core$Native_Scheduler.nativeBinding(function(callback) {
                var results = A2(update, message, model);
                model = results._0;
        ...
        let mainProcess = spawnLoop(initApp, onMessage);
    
        function enqueue(message)
        {
            _elm_lang$core$Native_Scheduler.rawSend(mainProcess, message);
        }
    
        let ports = setupEffects(managers, enqueue);
    ...
    function setupEffects(managers, callback)
    {
        ...
        for (let key in effectManagers)
        {
            let manager = effectManagers[key];
    ...
    function makeManager(info, callback)
    {
        ...
        let tag = info.tag;
        let onEffects = info.onEffects;
        let onSelfMessage = info.onSelfMsg;
    
        function onMessage(msg, state)
        {
            if (msg.ctor === 'self')
            {
                return A3(onSelfMessage, router, msg._0, state);
            }
        ...
        let process = spawnLoop(info.init, onMessage);
        router.self = process;
        return process;
    }
    
    function sendToApp(router, message)
    {
        return _elm_lang$core$Native_Scheduler.nativeBinding(function(callback)
        {
            router.main(message);
    ...
    function sendToSelf(router, message)
    {
    ...
    {
        let andThen = _elm_lang$core$Native_Scheduler.andThen;
    
        function loop(state)
        {
            let handleMessage = _elm_lang$core$Native_Scheduler.receive(function(msg) {
                return onMessage(msg, state);
            });
            return A2(andThen, handleMessage, loop);
        }
    
        let task = A2(andThen, init, loop);
    ...
    function dispatchEffects(managers, commandBag, subBag)
    {
        var effectsDict = {};
        gatherEffects(true, commandBag, effectsDict, null);
        gatherEffects(false, subBag, effectsDict, null);
    
        for (let home in managers)
        {
            let fx = home in effectsDict
                ? effectsDict[home]
                : {
    ...
    function gatherEffects(isCommand, bag, effectsDict, taggers)
    {
        switch (bag.type)
        {
            case 'leaf':
                var home = bag.home;
                var effect = toEffect(isCommand, home, taggers, bag.value);
                effectsDict[home] = insert(isCommand, effect, effectsDict[home]);
                return;
            ...
            case 'node':
                ...
                while (list.ctor !== '[]')
                {
                    gatherEffects(isCommand, list._0, effectsDict, taggers);
                    list = list._1;
                }
                return;
    
            case 'map':
                gatherEffects(isCommand, bag.tree, effectsDict, {
                    tagger: bag.tagger,
                    rest: taggers
                });
                return;
    ...
    function toEffect(isCommand, home, taggers, value)
    {
    ...
    function insert(isCommand, newEffect, effects)
    {
        effects = effects || {
            cmds: _elm_lang$core$Native_List.Nil,
            subs: _elm_lang$core$Native_List.Nil
    ...
    var outgoingPortMap = F2(function commandMap(tagger, value) {
        return value;
    });
    ...
    var incomingPortMap = F2(function subscriptionMap(tagger, finalTagger)
    {
        return function(value)
        {
    ...
    {
        let subscriptions = _elm_lang$core$Native_List.Nil;
        var converter = effectManagers[name].converter;
    ...
    _elm_lang$core$Platform_Command_ops['!'] = F2(
        function (model, commands) {
            return {
                ctor: '_Tuple2',
                _0: model,
                _1: _elm_lang$core$Platform_Command$batch(commands)
            };
        });
    ...
    let _evancz$elm_todomvc$Todo$updateWithStorage = F2(
        (msg, model) => {
            ...
            return {
                ctor: '_Tuple2',
                _0: newModel,
                _1: _elm_lang$core$Platform_Command$batch(
                    _elm_lang$core$Native_List.fromArray(
                        [
        ...
        (visibility, entries) => {
            let entriesCompleted = _elm_lang$core$List$length(
                A2(
                    ...
                    entries));
            let entriesLeft = _elm_lang$core$List$length(entries) - entriesCompleted;
            return A2(
                _elm_lang$html$Html$footer,
                _elm_lang$core$Native_List.fromArray(
                    [
                                                        ...
                                                        return A2(
                                                            ...
                                                            (id) => {
                                                                return _elm_lang$core$Json_Decode$succeed(
                                                                    {completed, description, editing, id});
                                                            });
                                                    });
                                            });
                                    }))),
