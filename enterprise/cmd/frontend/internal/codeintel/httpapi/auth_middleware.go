...
func authMiddleware(next http.Handler, db dbutil.DB, authValidators AuthValidatorMap, operation *obsv.Operation) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		statusCode, err := func() (_ int, err error) {
			ctx, traceLog, endObservation := operation.WithAndLogger(r.Context(), &err, obsv.Args{})
			...
			if !conf.Get().LsifEnforceAuth || isSiteAdmin(ctx, db) || hasQuery(r, "uploadId") {
				traceLog(obsv.Event("bypassing code host auth check"))
				return 0, nil
			}
			...
			for codeHost, validator := range authValidators {
				...
				traceLog(obsv.String("codeHost", codeHost))
