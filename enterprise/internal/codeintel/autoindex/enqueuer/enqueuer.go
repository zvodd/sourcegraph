...
func (s *IndexEnqueuer) InferIndexConfiguration(ctx context.Context, repositoryID int) (_ *config.IndexConfiguration, err error) {
	ctx, traceLog, endObservation := s.operations.InferIndexConfiguration.WithAndLogger(ctx, &err, obsv.Args{
	...
	traceLog(obsv.String("commit", commit))
...
func (s *IndexEnqueuer) QueueIndexes(ctx context.Context, repositoryID int, rev, configuration string, force bool) (_ []store.Index, err error) {
	ctx, traceLog, endObservation := s.operations.QueueIndex.WithAndLogger(ctx, &err, obsv.Args{
		LogFields: []obsv.Field{
			obsv.Int("repositoryID", repositoryID),
		},
	})
...
func (s *IndexEnqueuer) QueueIndexesForPackage(ctx context.Context, pkg precise.Package) (err error) {
	ctx, traceLog, endObservation := s.operations.QueueIndexForPackage.WithAndLogger(ctx, &err, obsv.Args{
		LogFields: []obsv.Field{
			obsv.String("scheme", pkg.Scheme),
			obsv.String("name", pkg.Name),
			obsv.String("version", pkg.Version),
		},
	})
	defer endObservation(1, obsv.Args{})
	...
	traceLog(obsv.String("repoName", repoName))
	traceLog(obsv.String("revision", revision))
...
// will cause this method to no-op. Note that this is NOT a guarantee that there will never be any duplicate records
// when the flag is false.
func (s *IndexEnqueuer) queueIndexForRepositoryAndCommit(ctx context.Context, repositoryID int, commit, configuration string, force bool, traceLog obsv.TraceLogger) ([]store.Index, error) {
	if !force {
	...
	traceLog(obsv.Int("numIndexes", len(indexes)))
